  ...
  switch (input.type) {
    case 'USAGE': {
      ...
      if (!Number.isInteger(input.quantity) || input.quantity < 1)
        fail('USAGE requires integer quantity >= 1');
    ...
    case 'PURCHASE': {
      if (!Number.isInteger(input.quantity) || input.quantity < 1)
        fail('PURCHASE requires integer quantity >= 1');
      quantity = input.quantity;
    ...
    case 'INITIAL': {
      if (!Number.isInteger(input.quantity) || input.quantity < 0)
        fail('INITIAL requires integer quantity >= 0');
      quantity = input.quantity;
      delta = input.quantity;
      break;
    }
    ...
    case 'ADJUSTMENT': {
      if (!Number.isInteger(input.delta) || input.delta === 0)
        fail('ADJUSTMENT requires a non-zero integer delta');
      delta = input.delta;
    ...
    case 'UNDO': {
      ...
      if (common.babyId !== input.original.babyId)
        fail('UNDO babyId does not match original');
      if (common.sizeId !== input.original.sizeId)
